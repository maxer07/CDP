...
(function () {
	...
	if (args.length < 1) {
		console.log('Usage: phantomjs highcharts-convert.js -scriptResources script1;script2;script3 -scriptFile tmp.js -highchartsModelVar highchartsConfig -outfile tmp.svg');
		phantom.exit(1);
	} else {
		input = "json";
		...
		scriptResources = args.scriptResources;
		highchartsModelVarName = args.highchartsModelVar;
		...
		if (scriptResources) {
			var arr = scriptResources.split(';'), i, ln = arr.length;
			for (i = 0; i< ln; i += 1) {
				console.log('injecting script: ' + arr[i]);
				page.injectJs(arr[i]);				
			}
		}

		if (scriptFile)	 {
			console.log('injecting scriptfile: ' + scriptFile);
			page.injectJs(scriptFile);			
		}
		...
		svg = page.evaluate(function (width, constr, callbackStr, pdfOutput, highchartsModelVarName) {
			var options = window[highchartsModelVarName],
				imagesLoadedMsg = 'Highcharts.imagesLoaded:7a7dfcb5df73aaa51e67c9f38c5b07cb', $container, chart,
		...
		}, width, constr, callbackStr, pdfOutput, highchartsModelVarName);
