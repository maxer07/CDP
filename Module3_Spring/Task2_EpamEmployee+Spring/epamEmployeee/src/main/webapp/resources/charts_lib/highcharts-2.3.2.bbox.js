...
function attr(elem, prop, value) {
	...
	if (isString(prop)) {
		...
		if (defined(value)) {
			...
			if(typeof batikCanvas !== 'undefined') {
				if(elem.svgIndex) batikCanvas.setAttribute(SVGElements[elem.svgIndex], prop, value);
				if(SVGTextElements[elem.svgIndex]) {
					batikCanvas.setAttribute(SVGTextElements[elem.svgIndex], prop, value);	
				}
			}
	...
	// else if prop is defined, it is a hash of key/value pairs
		...
		for (key in prop) {
			...
			if(typeof batikCanvas !== 'undefined') {
				if(elem.svgIndex) batikCanvas.setAttribute(SVGElements[elem.svgIndex], key, prop[key]);
				if(SVGTextElements[elem.svgIndex]) {
					batikCanvas.setAttribute(SVGTextElements[elem.svgIndex], key, prop[key]);	
				}
			}
		}
	}
...
SVGElement.prototype = {
	...
	init: function (renderer, nodeName) {
		...
		if(typeof batikCanvas !== 'undefined') {
			wrapper.element.svgIndex = svgIndex;
			SVGElements[svgIndex] = batikCanvas.createSVGElement(SVG_NS, nodeName);
			if(nodeName == 'text') {
				SVGTextElements[svgIndex] = batikCanvas.createSVGElement(SVG_NS, nodeName);
			}	
			svgIndex++;		
		}
		/**
	...
	css: function (styles) {
		...
		if (isIE && !hasSVG) { // legacy IE doesn't support setting style attribute
			...
			var svgFontSize;
			for (n in styles) {
				if(typeof batikCanvas !== 'undefined' && n == 'fontSize'){
					svgFontSize = pInt(styles[n]);
					//hasFontSize = true;
					//continue;
				}
			...
			if(hasFontSize) elemWrapper.attr('font-size', svgFontSize);
		}
	...
	getBBox: function(debug) {
		...
		if (!bBox) {
			...
			if (element.namespaceURI === SVG_NS || renderer.forExport) {
				...
				if(this.added || this.svgTextAdded) {
					if(typeof batikCanvas !== 'undefined' && element.svgIndex && SVGElements[element.svgIndex] && SVGTextElements[element.svgIndex]) {
						var sizeW = batikCanvas.getSize(SVGElements[element.svgIndex]);						
						var sizeH = batikCanvas.getSize(SVGTextElements[element.svgIndex]); 
						bBox.width = sizeW[0];
						bBox.height = sizeH[1];
						debug && print(bBox.width + ',' + bBox.height);
					}
				} else {
					debug && print("Failed to get label size for " + element.tagName);
				}
	...
	add: function (parent, debug) {
		...
		if(typeof batikCanvas !== 'undefined') {
			 var SVGParentNode = parentWrapper.box ? renderer.boxSVGElement : SVGElements[parentNode.svgIndex];
			 var SVGElement = SVGElements[element.svgIndex];
		}
		...
		if (parentWrapper.handleZ) { // this element or any of its siblings has a z index
			for (i = 0; i < childNodes.length; i++) {
				...
				if (otherElement !== element && (
					...
					if(typeof batikCanvas !== 'undefined') {
						SVGParentNode.insertBefore(SVGElement, batikCanvas.getChildNode(SVGParentNode, i));	
					}			
					inserted = true;
					break;
				}
...
SVGRenderer.prototype = {
	...
	init: function (container, width, height, forExport) {
		...
		if(batikCanvas) {
			 renderer.boxSVGElement = batikCanvas.boxSVGElement;
		}
	 ...
	 */
	buildText: function(wrapper, debug) {
		...
		while (i--) {
			...
			if(typeof batikCanvas !== 'undefined') {
				batikCanvas.removeChildNode(SVGElements[textNode.svgIndex], batikCanvas.getChildNode(SVGElements[textNode.svgIndex], i));
				var childNode = batikCanvas.getChildNode(SVGTextElements[textNode.svgIndex], i);
				childNode && batikCanvas.removeChildNode(SVGTextElements[textNode.svgIndex], childNode);							
			}
		}

		if(typeof batikCanvas !== 'undefined') {
			if(typeof textNode.svgIndex !== 'undefined') {
				this.boxSVGElement.appendChild(SVGElements[textNode.svgIndex]);
				this.boxSVGElement.appendChild(SVGTextElements[textNode.svgIndex]);	
				wrapper.svgTextAdded = true;						
			} else {
				//print('textnode is not setup');
			}				
		}
		...
		each(lines, function (line, lineNo) {
			...
			each(spans, function (span) {
				if (span !== '' || spans.length === 1) {
					...
					if(typeof batikCanvas !== 'undefined') {
						tspan.svgIndex = svgIndex;
						SVGElements[svgIndex] = batikCanvas.createSVGElement(SVG_NS, 'tspan');
						SVGTextElements[svgIndex] = batikCanvas.createSVGElement(SVG_NS, 'tspan');	
						svgIndex++;		
					}
					...
					if(typeof batikCanvas !== 'undefined') {
						SVGElements[tspan.svgIndex].appendChild(batikCanvas.createSVGTextNode(span));
						SVGTextElements[tspan.svgIndex].appendChild(batikCanvas.createSVGTextNode(batikSpan));
					}
					...
					if(typeof batikCanvas !== 'undefined') {
						SVGElements[textNode.svgIndex].appendChild(SVGElements[tspan.svgIndex]);
						SVGTextElements[textNode.svgIndex].appendChild(SVGTextElements[tspan.svgIndex]);
					}		
					spanNo++;
					...
					if (width) {
						...
						while (words.length || rest.length) {
							...
							if (!tooLong || words.length === 1) { // new line needed
                                ...
                                if(typeof batikCanvas !== 'undefined') {
                                    batikCanvas.removeChildNode(SVGElements[tspan.svgIndex], SVGElements[tspan.svgIndex].getFirstChild());
                                    batikCanvas.removeChildNode(SVGTextElements[tspan.svgIndex], SVGTextElements[tspan.svgIndex].getFirstChild());
                                }
							}
							if (words.length) {
								tspan.appendChild(doc.createTextNode(words.join(' ').replace(/- /g, '-')));
                                if(typeof batikCanvas !== 'undefined') {
                                    SVGElements[tspan.svgIndex].appendChild(batikCanvas.createSVGTextNode(words.join(' ').replace(/- /g, '-')));
                                    SVGTextElements[tspan.svgIndex].appendChild(batikCanvas.createSVGTextNode(words.join(' ').replace(/- /g, '-') + '\u00c0p'));
                                }
							}
						}
					}
 ...
 */
function Legend(chart) {this.init(chart)}
...
Chart.prototype = {
	...
	render: function () {
		...
		// Title
		//(typeof batikCanvas !== 'undefined') && batikCanvas.setStatus('6:rendering title...');
		chart.setTitle();
		...
		// set axes scales
		//(typeof batikCanvas !== 'undefined') && batikCanvas.setStatus('7:rendering axis...');
		each(axes, function (axis) {
			axis.setScale();
		});
		chart.getMargins();
		...
		// The series
		var seriesIndex = 0;
		var seriesLen = chart.series ? chart.series.length : 0;
		//(typeof batikCanvas !== 'undefined') && batikCanvas.setStatus('8:rendering series...');
		...
		each(chart.series, function (serie) {
			//(typeof batikCanvas !== 'undefined') && batikCanvas.setStatus('9:rendering series ' + seriesIndex + ' / ' + seriesLen + '...')
			serie.translate();
		...
		// Labels
		//(typeof batikCanvas !== 'undefined') && batikCanvas.setStatus('10:rendering labels...');
		if (labels.items) {
	...
	firstRender: function () {
		...
		//(typeof batikCanvas !== 'undefined') && batikCanvas.setStatus('1:reset margins...');
		chart.resetMargins();
		//(typeof batikCanvas !== 'undefined') && batikCanvas.setStatus('2:set chart size...');
		chart.setChartSize();
		//(typeof batikCanvas !== 'undefined') && batikCanvas.setStatus('3:prop from series...');
		chart.propFromSeries();
		//(typeof batikCanvas !== 'undefined') && batikCanvas.setStatus('4:get axes...');
		chart.getAxes();

		// Initialize the series
		var seriesIndex = 0;
		var seriesLen = options.series ? options.series.length : 0;
		each(options.series || [], function (serieOptions) {
			//(typeof batikCanvas !== 'undefined') && batikCanvas.setStatus('5:init series ' + seriesIndex + ' / ' + seriesLen + '...');
		...
		if(typeof batikCanvas !== 'undefined') {
			for(var i=0;i<SVGTextElements.length;i++) {
				try {
					SVGTextElements[i] && chart.renderer.boxSVGElement.removeChild(SVGTextElements[i]);		
				} catch(err){
